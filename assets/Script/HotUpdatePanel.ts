...
@ccclass
@PanelPathConfig('common/HotUpdatePanel','cover')
export default class HotUpdatePanel extends BaseUIPanel {

    @property(cc.Label)
    mTipLabel: cc.Label = null;
    @property(cc.Label)
    mVersionLabel: cc.Label = null;

    @property(cc.ProgressBar)
    mTipProgressBar: cc.ProgressBar = null;
    ...
    versionCompareHandle:(varsionA:string,versionB:string)=>number;
    _am:any;
    ...
    checkCb(event) {
        ...
        switch (event.getEventCode())
        {
            case jsb.EventAssetsManager.ERROR_NO_LOCAL_MANIFEST:
                this.mTipLabel.string = "No local manifest file found, hot update skipped.";
                break;
            case jsb.EventAssetsManager.ERROR_DOWNLOAD_MANIFEST:
            case jsb.EventAssetsManager.ERROR_PARSE_MANIFEST:
                this.mTipLabel.string = "Fail to download manifest file, hot update skipped.";
                break;
            ...
            case jsb.EventAssetsManager.NEW_VERSION_FOUND:
                ...
                this.mTipLabel.string = 'New version found, please try to update.' + newVersion;
    ...
    updateCb(event) {
        ...
        switch (event.getEventCode())
        {
            ...
            case jsb.EventAssetsManager.UPDATE_PROGRESSION:
                this.mTipProgressBar.progress = event.getPercent();
                ...
                if (msg) {
                    this.mTipLabel.string = 'Updated file: ' + msg;
            ...
            case jsb.EventAssetsManager.UPDATE_FINISHED:
                this.mTipLabel.string = 'Update finished. ' + event.getMessage();
                needRestart = true;
                break;
            case jsb.EventAssetsManager.UPDATE_FAILED:
                this.mTipLabel.string = 'Update failed. ' + event.getMessage();
                // this.panel.retryBtn.active = true;
                this._updating = false;
                this._canRetry = true;
                break;
            case jsb.EventAssetsManager.ERROR_UPDATING:
                this.mTipLabel.string = 'Asset update error: ' + event.getAssetId() + ', ' + event.getMessage();
                break;
            case jsb.EventAssetsManager.ERROR_DECOMPRESS:
                this.mTipLabel.string = event.getMessage();
                break;
            default:
                break;
        }
    ...
    loadCustomManifest() {
        if (this._am.getState() === jsb.AssetsManager.State.UNINITED) {
            ...
            this.mTipLabel.string = 'Using custom manifest';
        }
    }
    ...
    retry () {
        if (!this._updating && this._canRetry) {
            ...
            this.mTipLabel.string = 'Retry failed Assets...';
    ...
    checkUpdate () {
        if (this._updating) {
            this.mTipLabel.string = 'Checking or updating ...';
            return;
        }
        if (this._am.getState() === jsb.AssetsManager.State.UNINITED) {
            // Resolve md5 url
            ...
            this.mVersionLabel.string = 'V' + this._am.getLocalManifest().getVersion();
        }
        if (!this._am.getLocalManifest() || !this._am.getLocalManifest().isLoaded()) {
            this.mTipLabel.string = 'Failed to load local manifest ...';
            return;
        }
